<?php
/**
 * Classe d'accÃ¨s aux donnÃ©es et plus encore...
...
class PdoSeminaire{
	...
	// TODO : Ã  prendre dans une table de constantes, ou Ã  dÃ©terminer selon les roles de participants
	...
	/**
	 *
	 * Obtient des informations (nombre d'inscrits...) sur les seances d'un mÃªme crÃ©neau (dateHeureDebut)
	...
	/**
	 *
	 * Obtient le nombre de sÃ©ances auxquelles une personne donnÃ©e s'est inscrites
	...
	/**
	 *
	 * Inscrit une personne Ã  une sÃ©ance. Garantit que cette personne
	 * ne sera pas inscrite sur une sÃ©ance d'un mÃªme horaire (heure de debut)
	 * @param int $idSeance
	 * @param int $idParticipant
	 */
	public function inscriptionSeance($idSeance, $idParticipant) {
		try {
			...
			// supprime toutes les inscriptions du participant Ã  l'heure
			// de la sÃ©ance Ã  laquelle il demande l'inscription
	...
	/**
	 * Supprime toutes les inscriptions d'un participant Ã  une date donnÃ©e
	 * @param date_sql $dateHeureDebut
	 * @param int $idParticipant
	 */
	...
	/**
	 *
	 * obtient la liste des seances inscrites liÃ©es Ã  un participant
	 * @param int $idParticipant le participant concernÃ©
	 */
	...
	public function razInscriptionSeances($dateHeureDebut, $idParticipant) {
		try {
			// supprime toutes les inscriptions d'un participant Ã  toutes
			// les sÃ©ances d'une heure donnÃ©e
...
*/
	...
	public function envoyerMail(){
		...
		if ($user) {
		 ...
		 $headers .='Content-Type: text/plain; charset="UTF-8"'."\n";
		 $headers .='Content-Transfer-Encoding: 8bit';
		 $message = "Vous Ãªtes inscrit Ã  : " . $url;
		 if ($user->seminaire) {
		 	$message .= "\nPour le seminaire : " . $user->seminaire->nom;		 	
		 }
		 $message .= "\nNom  : " . $user->prenom . ' ' . $user->nom;
